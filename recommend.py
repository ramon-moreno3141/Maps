...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    key_value_pairs=[[find_closest(restaurant_location(r),centroids),r] for r in restaurants]
    return group_by_first(key_value_pairs)
    # END Question 4
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xs_ys_pairs=zip(xs,ys)
    Sxy=sum([(Xi-mean(xs))*(Yi-mean(ys)) for Xi,Yi in xs_ys_pairs])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_r_squared_pairs=[find_predictor(user,reviewed,f) for f in feature_fns]
    max_pair=max(predictor_r_squared_pairs, key=lambda pair: pair[1])
    return max_pair[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    known_ratings=[[restaurant_name(r),user_rating(user,restaurant_name(r))] for r in reviewed]
    predicted_ratings=[[restaurant_name(r),predictor(r)] for r in restaurants if r not in reviewed]
    all_ratings=known_ratings + predicted_ratings
